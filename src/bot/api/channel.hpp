...
namespace bot {
    namespace api {
        namespace channel {
            datatypes::message::message_t SendMessage(const std::string& channel_id, const std::string& content, bool tts = false) {
            ...
            datatypes::message::message_t EditMessage(const std::string& channel_id, const std::string& message_id, const std::string& content, bool tts = false) {
                rapidjson::Document json;
                json.SetObject();
                rapidjson::Value content_(content.c_str(), content.size(), json.GetAllocator());
                json.AddMember("content", content_, json.GetAllocator());
